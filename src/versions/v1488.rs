...
pub(crate) fn register(types: &MinecraftTypesMut) {
    ...
    rename_item(types, VERSION, simple_rename("minecraft:kelp_top", "minecraft:kelp"));
    ...
    types.tile_entity.borrow_mut().add_converter_for_id("minecraft:command_block", VERSION, map_data_converter_func(|data, _from_version, _to_version| {
        v1458::update_custom_name(data);
    }));

    types.entity.borrow_mut().add_converter_for_id("minecraft:commandblock_minecart", VERSION, map_data_converter_func(|data, _from_version, _to_version| {
        v1458::update_custom_name(data);
    }));
    ...
    types.structure_feature.borrow_mut().add_structure_converter(VERSION, map_data_converter_func(|data, _from_version, _to_version| {
        let Some(Value::List(children)) = data.get_mut("Children") else { return };

        let is_child_igloo = |child: ValueRef| {
            let ValueRef::Compound(child) = child else { return false };
            let Some(Value::String(id)) = child.get("id") else { return false };
            id == "Iglu"
        };
