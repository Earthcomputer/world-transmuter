use rust_dataconverter_engine::{convert_map_list_in_map, convert_object_in_map, convert_object_list_in_map, convert_values_in_map, data_walker, map_data_converter_func, value_data_converter_func};
...
pub(crate) fn register(types: &MinecraftTypesMut) {
    ...
    types.chunk.borrow_mut().add_structure_converter(VERSION, map_data_converter_func(|data, _from_version, _to_version| {
        fn move_out_of_bound_ticks(ticks_key: &str, chunk_root: &mut Compound, chunk_x: i32, chunk_z: i32, into_key: &str) {
            let Some(Value::List(List::Compound(ticks))) = chunk_root.get_mut(ticks_key) else { return };
            ...
            if !out_of_bounds.is_empty() {
                ...
                let Some(Value::Compound(upgrade_data)) = chunk_root.get_mut("UpgradeData") else { unreachable!() };
    ...
    types.chunk.borrow_mut().add_structure_walker(VERSION, data_walker(move |data, from_version, to_version| {
        ...
        convert_map_list_in_map(tile_entity_type, data, "block_entities", from_version, to_version);

        if let Some(Value::List(List::Compound(block_ticks))) = data.get_mut("block_ticks") {
            for block_tick in block_ticks {
                convert_object_in_map(block_name_type, block_tick, "i", from_version, to_version);
            }
        }
        ...
        if let Some(Value::Compound(upgrade_data)) = data.get_mut("UpgradeData") {
            ...
            if let Some(Value::List(List::Compound(neighbor_block_ticks))) = upgrade_data.get_mut("neighbor_block_ticks") {
