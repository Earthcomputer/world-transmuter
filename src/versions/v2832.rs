...
use log::error;
use rust_dataconverter_engine::{convert_map_list_in_map, convert_object_in_map, convert_object_list_in_map, convert_values_in_map, data_walker, get_mut_multi, map_data_converter_func};
use valence_nbt::{compound, Compound, List, Value};
use crate::helpers::bit_storage::{AlignedBitStorage, ceil_log2, LocalPos, NullSectionInitializer, Section};
...
fn resize(val: &[i64], old_bits_per_object: u8, new_bits_per_object: u8) -> Result<Vec<i64>, String> {
    ...
    let items = 4096;
    let mut ret = Vec::with_capacity((items + new_objects_per_value as usize - 1) / new_objects_per_value as usize);

    let expected_size = (items + old_objects_per_value as usize - 1) / old_objects_per_value as usize;
    if val.len() != expected_size {
        return Err(format!("Expected size: {}, got: {}", expected_size, val.len()));
    }
    ...
    for &old_curr in val {
        ...
        while curr_item < items && obj_idx + old_bits_per_object <= 64 {
            ...
            if shift + new_bits_per_object > 64 { // will next write overflow?
    ...
    assert_eq!(ret.len(), (items + new_objects_per_value as usize - 1) / new_objects_per_value as usize);
...
fn fix_lithium_chunks(data: &mut Compound) {
    ...
    for section in sections {
        ...
        let [Some(Value::List(List::Compound(palette))), Some(Value::LongArray(block_states))] = get_mut_multi(section, ["Palette", "BlockStates"]) else { continue };
        ...
        match resize(block_states, got_bits, expected_bits) {
            ...
            Err(message) => error!("Failed to rewrite mismatched palette and data storage for section y: {} \
                for chunk [{},{}], palette entries: {}, data storage size: {}: {}",
                section_y, chunk_x, chunk_z, palette.len(), block_states.len(), message),
        }
    }
}
...
pub(crate) fn register(types: &MinecraftTypesMut) {
    ...
    types.chunk.borrow_mut().add_structure_converter(VERSION, map_data_converter_func(|data, _from_version, _to_version| {
        ...
        let [Some(Value::Compound(level)), context] = get_mut_multi(data, ["Level", "__context"]) else { return };
        ...
        let mut new_biomes = create_biome_sections(level, is_overworld, &mut is_already_extended);
        ...
        let Some(Value::List(sections)) = level.get_mut("Sections") else { unreachable!() };
        ...
        if let List::Compound(sections) = sections {
            for (idx, section) in sections.iter_mut().enumerate() {
                ...
                if (0..new_biomes.len() as i32).contains(&section_index) {
                    ...
                    section.insert("biomes", std::mem::take(&mut new_biomes[section_index as usize]));
                }
                ...
                if let (Some(palette), Some(block_states)) = (palette, block_states) {
                    section.insert("block_states", wrap_palette_optimized(palette, block_states));
                } else {
        ...
        upgrade_chunk_data(level, is_overworld, is_already_extended, generator == "minecraft:noise", bottom_section_idx);
    }));

    let biome_type = types.biome;
    let block_name_type = types.block_name;
    let multi_noise_biome_source_parameter_list_type = types.multi_noise_biome_source_parameter_list;
    types.world_gen_settings.borrow_mut().add_structure_walker(VERSION, data_walker(move |data, from_version, to_version| {
        let Some(Value::Compound(dimensions)) = data.get_mut("dimensions") else { return };
        for dimension_data in dimensions.values_mut() {
            let Value::Compound(dimension_data) = dimension_data else { continue };
            let Some(Value::Compound(generator)) = dimension_data.get_mut("generator") else { continue };
            let Some(Value::String(typ)) = generator.get("type") else { continue };

            match &typ[..] {
                "minecraft:flat" => {
                    let Some(Value::Compound(settings)) = generator.get_mut("settings") else { continue };

                    convert_object_in_map(biome_type, settings, "biome", from_version, to_version);

                    if let Some(Value::List(List::Compound(layers))) = settings.get_mut("layers") {
                        for layer in layers {
                            convert_object_in_map(block_name_type, layer, "block", from_version, to_version);
                        }
                    }
                }
                "minecraft:noise" => {
                    if let Some(Value::Compound(settings)) = generator.get_mut("settings") {
                        convert_object_in_map(block_name_type, settings, "default_block", from_version, to_version);
                        convert_object_in_map(block_name_type, settings, "default_fluid", from_version, to_version);
                    }

                    if let Some(Value::Compound(biome_source)) = generator.get_mut("biome_source") {
                        if let Some(Value::String(biome_source_type)) = biome_source.get("type") {
                            match &biome_source_type[..] {
                                "minecraft:fixed" => {
                                    convert_object_in_map(biome_type, biome_source, "biome", from_version, to_version);
                                }
                                "minecraft:multi_noise" => {
                                    convert_object_in_map(multi_noise_biome_source_parameter_list_type, biome_source, "preset", from_version, to_version);
                                    ...
                                    if let Some(Value::List(List::Compound(biomes))) = biome_source.get_mut("biomes") {
                                        for biome in biomes {
                                            convert_object_in_map(biome_type, biome, "biome", from_version, to_version);
                                        }
                                    }
                                }
                                "minecraft:checkerboard" => {
                                    convert_object_list_in_map(biome_type, biome_source, "biomes", from_version, to_version);
                                }
                                _ => {}
                            }
                        }
        ...
        if let Some(Value::List(List::Compound(sections))) = level.get_mut("Sections") {
            for section in sections {
                if let Some(Value::Compound(biomes)) = section.get_mut("biomes") {
                    convert_object_list_in_map(biome_type, biomes, "palette", from_version, to_version);
                }
                if let Some(Value::Compound(block_states)) = section.get_mut("block_states") {
                    convert_map_list_in_map(block_state_type, block_states, "palette", from_version, to_version);
                }
            }
        }

        if let Some(Value::Compound(structures)) = level.get_mut("Structures") {
            convert_values_in_map(structure_feature_type, structures, "Starts", from_version, to_version);
        }
    }));
}
...
fn predict_chunk_status_before_surface(level: &mut Compound, mut chunk_blocks: BTreeSet<String>) {
    ...
    let chunk_feature_status = chunk_blocks.into_iter().any(|block| !blocks_before_feature_status().contains(&block[..]));
...
fn get_empty_block_palette() -> Compound {
    wrap_palette(List::Compound(vec![compound! {
        "Name" => "minecraft:air",
    }]), None)
}

fn shift_upgrade_data(upgrade_data: Option<&mut Value>, shift: i32) {
    let Some(Value::Compound(upgrade_data)) = upgrade_data else { return };
    let Some(Value::Compound(indices)) = upgrade_data.get_mut("Indices") else { return };

    rust_dataconverter_engine::rename_keys(indices, |input| input.parse::<i32>().ok().map(|i| (i + shift).to_string()));
}

fn upgrade_chunk_data(level: &mut Compound, want_extended_height: bool, is_already_extended: bool, on_noise_generator: bool, bottom_section_idx: Option<usize>) {
    level.remove("Biomes");
    ...
    let Some(Value::String(status)) = level.get("Status") else { return };
    if status == "empty" {
        return;
    }
    ...
    level.insert("blending_data", compound! {
        "old_noise" => old_noise,
    });

    let Some(bottom_section_idx) = bottom_section_idx else { return };
    let Some(Value::List(List::Compound(sections))) = level.get("Sections") else { unreachable!("bottom_section_idx can only get a value if there is a sections list and it is non-empty") };
    let bottom_section = &sections[bottom_section_idx];

    let chunk_x = level.get("xPos").and_then(|v| v.as_i32()).unwrap_or(0);
    let chunk_z = level.get("zPos").and_then(|v| v.as_i32()).unwrap_or(0);
    let Some(bottom_section) = Section::<AlignedBitStorage<&[i64]>>::wrap_2832(chunk_x, chunk_z, bottom_section, &mut NullSectionInitializer) else { return };

    let mut missing_bedrock: BitArray<_, Lsb0> = BitArray::new([0u64; 4]);
    let mut has_bedrock = status == "noise";
    for z in 0..=15 {
        for x in 0..=15 {
            let Some(block) = bottom_section.get_block(LocalPos::new(x, 0, z)) else { continue };
    ...
    if has_bedrock && missing_bedrock != [u64::MAX; 4] {
        let target_status = if status == "full" { "heightmaps".to_owned() } else { status };
        level.insert("below_zero_retrogen", compound! {
...
fn pad_carving_masks(level: &mut Compound, new_size: usize, offset: usize) {
    ...
    for carving_mask in carving_masks.values_mut() {
        let Value::ByteArray(old) = carving_mask else { continue };
        let mut new_val = vec![0; 64 * new_size];
        i8_slice_to_i64_le(&old[..((64 * new_size - 64 * offset) * 8).min(old.len())], &mut new_val[..], 64 * offset);
        *carving_mask = Value::LongArray(new_val);
    }
}

fn i8_slice_to_i64_le(i8_slice: &[i8], i64_slice: &mut [i64], offset: usize) {
    let u8_slice = unsafe { std::slice::from_raw_parts(i8_slice.as_ptr() as *const u8, i8_slice.len()) };
    ...
    for i in 0..u8_slice.len() / 8 {
        unsafe {
            ...
            *i64_slice.get_unchecked_mut(offset + i) = i64::from_le_bytes(sub_slice.try_into().unwrap());
        }
    }
}
...
fn create_biome_sections(level: &Compound, want_extended_height: bool, is_already_extended: &mut bool) -> Vec<Compound> {
    ...
    if let Some(biomes) = biomes {
        if biomes.len() == 1536 { // magic value for 24 sections of biomes (24 * 4^3)
        ...
        if biomes.len() == 1024 { // magic value for 16 sections of biomes (16 * 4^3)
            if want_extended_height {
...
fn create_biome_section(biomes: &[i32], offset: usize, mask: usize) -> Compound {
    ...
    let mut packed = Vec::with_capacity((64 + objects_per_value as usize - 1) / objects_per_value as usize);
    ...
    for biome_idx in 0..64 {
        ...
        if shift + bits_per_object > 64 { // will next write overflow?
            // must move to next idx
    ...
    assert_eq!(packed.len(), (64 + objects_per_value as usize - 1) / objects_per_value as usize);
