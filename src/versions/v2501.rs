...
fn register_furnace(types: &MinecraftTypesMut, id: &str) {
    types.tile_entity.borrow_mut().add_converter_for_id(id, VERSION, map_data_converter_func(|data, _from_version, _to_version| {
        let recipes_used_size = data.remove("RecipesUsedSize").and_then(|o| o.as_i32()).unwrap_or(0);
        ...
        for i in 0..recipes_used_size {
            if let (Some(Value::String(recipe_key)), Some(recipe_amount)) = (data.get(&format!("RecipeLocation{}", i)), data.get(&format!("RecipeAmount{}", i)).and_then(|v| v.as_i32())) {
                if recipe_amount > 0 {
    ...
    types.tile_entity.borrow_mut().add_walker_for_id(VERSION, id, data_walker(move |data, from_version, to_version| {
