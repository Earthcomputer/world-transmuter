...
pub(crate) fn register(types: &MinecraftTypesMut) {
    types.entity.borrow_mut().add_structure_converter(VERSION, map_data_converter_func(move |data, _from_version, _to_version| {
            ...
            if let Some(hand_item) = equipment.next() {
                if !matches!(data.get("HandItems"), Some(Value::List(_))) {
                    if let Value::Compound(hand_item) = hand_item {
                        data.insert("HandItems", List::Compound(vec![
                            hand_item,
                            Compound::new(),
                        ]));
                    }
    ...
    types.structure.borrow_mut().add_structure_walker(VERSION, data_walker(move |data, from_version, to_version| {
