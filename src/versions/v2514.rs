...
macro_rules! replace_uuid_least_most {
    ($data:expr, $prefix:literal, $new_path:literal) => {
        replace_uuid_from_longs($data, concat!($prefix, "Least"), concat!($prefix, "Most"), $new_path)
    }
}

pub(crate) fn replace_uuid_from_longs(data: &mut Compound, least: &str, most: &str, new_path: &str) {
...
fn replace_uuid_string(data: &mut Compound, path: &str, new_path: &str) {
    let Some(Value::String(uuid)) = data.get(path) else { return };
    let Ok(uuid) = uuid.parse::<uuid::Uuid>() else { return };
...
pub(crate) fn register(types: &MinecraftTypesMut) {
    ...
    for id in [
        ...
        types.entity.borrow_mut().add_converter_for_id(id, VERSION, map_data_converter_func(|data, _from_version, _to_version| {
    ...
    types.entity.borrow_mut().add_converter_for_id("minecraft:armor_stand", VERSION, map_data_converter_func(|data, _from_version, _to_version| {
    ...
    types.entity.borrow_mut().add_converter_for_id("minecraft:fox", VERSION, map_data_converter_func(|data, _from_version, _to_version| {
        update_fox(data);
    }));
    types.entity.borrow_mut().add_converter_for_id("minecraft:item", VERSION, map_data_converter_func(|data, _from_version, _to_version| {
        update_item(data);
    }));
    types.entity.borrow_mut().add_converter_for_id("minecraft:shulker_bullet", VERSION, map_data_converter_func(|data, _from_version, _to_version| {
        update_shulker_bullet(data);
    }));
    types.entity.borrow_mut().add_converter_for_id("minecraft:area_effect_cloud", VERSION, map_data_converter_func(|data, _from_version, _to_version| {
    ...
    types.entity.borrow_mut().add_converter_for_id("minecraft:evoker_fangs", VERSION, map_data_converter_func(|data, _from_version, _to_version| {
        update_evoker_fangs(data);
    }));
    types.entity.borrow_mut().add_converter_for_id("minecraft:piglin", VERSION, map_data_converter_func(|data, _from_version, _to_version| {
        update_piglin(data);
    }));

    // Update TE
    types.tile_entity.borrow_mut().add_converter_for_id("minecraft:conduit", VERSION, map_data_converter_func(|data, _from_version, _to_version| {
        replace_uuid_ml_tag(data, "target_uuid", "Target");
    }));
    types.tile_entity.borrow_mut().add_converter_for_id("minecraft:skull", VERSION, map_data_converter_func(|data, _from_version, _to_version| {
    ...
    // Player UUID
    types.player.borrow_mut().add_structure_converter(VERSION, map_data_converter_func(|data, _from_version, _to_version| {
        update_living_entity(data);
        replace_uuid_least_most!(data, "UUID", "UUID");
    ...
    types.level.borrow_mut().add_structure_converter(VERSION, map_data_converter_func(|data, _from_version, _to_version| {
        ...
        if let Some(Value::Compound(dimension_data)) = data.get_mut("DimensionData") {
            for dimension_data in dimension_data.values_mut() {
                ...
                if let Some(Value::Compound(dragon_fight)) = dimension_data.get_mut("DragonFight") {
                ...
                if let Value::Compound(custom_boss_event) = custom_boss_event {
                    if let Some(Value::List(players)) = custom_boss_event.get_mut("Players") {
                        let new_players: Vec<_> = match players {
                            List::Compound(players) => {
                                players.iter().filter_map(|player| {
                                    let least = player.get("L").and_then(|v| v.as_i64()).unwrap_or(0);
                                    let most = player.get("M").and_then(|v| v.as_i64()).unwrap_or(0);
                                    if least != 0 && most != 0 {
    ...
    types.saved_data.borrow_mut().add_structure_converter(VERSION, map_data_converter_func(|data, _from_version, _to_version| {
        let Some(Value::Compound(data)) = data.get_mut("data") else { return };
        let Some(Value::List(List::Compound(raids))) = data.get_mut("Raids") else { return };
        for raid in raids {
            if let Some(Value::List(heroes)) = raid.get_mut("HeroesOfTheVillage") {
                let new_heroes: Vec<_> = match heroes {
                    List::Compound(heroes) => {
                        heroes.iter().filter_map(|hero| {
                            let least = hero.get("UUIDLeast").and_then(|v| v.as_i64()).unwrap_or(0);
                            let most = hero.get("UUIDMost").and_then(|v| v.as_i64()).unwrap_or(0);
                            if least != 0 || most != 0 {
                                Some(create_uuid_from_longs(least, most))
                            } else {
                                None
...
fn update_fox(data: &mut Compound) {
    if let Some(Value::List(trusted_uuids)) = data.remove("TrustedUUIDs") {
        let trusted: Vec<_> = match trusted_uuids {
            List::Compound(trusted_uuids) => {
                trusted_uuids.iter().filter_map(|uuid| {
        ...
        data.insert("Trusted", if trusted.is_empty() { List::End } else { List::IntArray(trusted) });
    }
}
...
fn update_piglin(data: &mut Compound) {
    let Some(Value::Compound(brain)) = data.get_mut("Brain") else { return };
    let Some(Value::Compound(memories)) = brain.get_mut("memories") else { return };
    let Some(Value::Compound(angry_at)) = memories.get_mut("minecraft:angry_at") else { return };
