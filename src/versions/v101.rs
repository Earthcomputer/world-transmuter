...
pub(crate) fn register(types: &MinecraftTypesMut) {
    types.tile_entity.borrow_mut().add_converter_for_id("Sign", VERSION, map_data_converter_func(|data, _from_version, _to_version| {
...
fn update_line(data: &mut Compound, key: &str) {
    let Some(Value::String(str)) = data.get(key) else { return };
