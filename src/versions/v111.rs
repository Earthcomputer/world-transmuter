...
pub(crate) fn register(types: &MinecraftTypesMut) {
    types.entity.borrow_mut().add_converter_for_id("Painting", VERSION, map_data_converter_func(|data, _from_version, _to_version| {
        convert_entity_rotation(data, false);
    }));
    types.entity.borrow_mut().add_converter_for_id("ItemFrame", VERSION, map_data_converter_func(|data, _from_version, _to_version| {
...
fn convert_entity_rotation(data: &mut Compound, is_item_frame: bool) {
    ...
    let facing = if let Some(direction) = data.remove("Direction").and_then(|o| o.as_i8()) {
        ...
        data.insert("TileX", data.get("TileX").and_then(|v| v.as_i32()).unwrap_or(0) + x_off);
        data.insert("TileZ", data.get("TileZ").and_then(|v| v.as_i32()).unwrap_or(0) + z_off);
        if is_item_frame {
